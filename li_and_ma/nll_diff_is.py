# -*- coding: utf-8 -*-
"""
Created on Thu Sep 26 12:18:52 2024
...
def nll_diff_is(x, trl, stiPar, config_pool,subject,  condIdx=None,savefile=True,return_nllk_only=True,renormed=False):
    """
    ...
    and computes the negative log-likelihood of the model.

    Args:
    - x: parameter vector [boundaries, sensory noise, inference noise, lapse rate]
    - trl: dictionary containing trial data (with 'estD', 'estC', 'config', 'target_x', 'target_y')
    - stiPar: dictionary containing stimulus parameters (ux, uy, sig_s, nr, ncat)
    - config_pool: list of configurations to consider
    - condIdx: indices for conditioning trials (default: all trials)
    ...
    #print("lapse", lapse, "temperature", T)
    ...
    for idxconfig,config in enumerate(config_pool):
        ...
        # pMat_dec[config-1] = np.empty((ncat, ntrl))
        # pMat_conf[config-1] = np.empty((nr, ntrl))
        pMat_dec[idxconfig] = np.empty((ncat, ntrl))
        pMat_conf[idxconfig] = np.empty((nr, ntrl))
        ...
        temp = np.sort(sim_post, axis=2)
        if renormed:
            ####################################
            ## if renormlising the 2 options before taking the diff
            temp = temp / np.repeat(np.sum(temp[:,:,1:],axis=2)[:,:,np.newaxis],temp.shape[2], axis=2)  ### this is renormlising across top 2 before taking the diff 
            #####################################
        sim_c_diff = temp[:, :, -1] - temp[:, :, -2]  # DIFF confidence
        ...
        sim_r = np.digitize(sim_c_diff, k, right=True)
        ...
        #plot_model(temp_s,temp_sy,config,model_confidence, true_confidence,"Diff",subject,k)
        if "T" in locals() and T!=1:
            plot_model(temp_s,temp_sy,config,model_confidence, true_confidence,"DiffwithTemp",subject,k, T=T,savefile=savefile)
        else: 
            plot_model(temp_s,temp_sy,config,model_confidence, true_confidence,"Diff",subject,k, savefile=savefile)
