...
def comp(p, q):
    ...
    mask = (p < 1) ############PK changed so that don't mask
...
comp2 = comp(posterior2, p3) # pk changed
comp3 = comp(posterior3, p3) # pk changed
comp4 = comp(posterior4, p3) # pk changed
complexities = [comp1,comp2,comp3,comp4]


Fs = np.array(complexities)-np.array(accuracies)

print("Accy", accuracies)
print("Complexity", complexities)
